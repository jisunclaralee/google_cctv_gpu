...
- **Q. Git ?? ?? PATH ??? ?? ??? ??? ??????**  
  ?? PowerShell ???? ? PATH? ???? ?? `refreshenv` ???? ?? 1) ???? ?? ?? PATH? ????? 2) ?? ???? `$env:PATH = [System.Environment]::GetEnvironmentVariable("PATH","Machine") + ";" + [System.Environment]::GetEnvironmentVariable("PATH","User")` ? `git --version`?? ???? ???.
